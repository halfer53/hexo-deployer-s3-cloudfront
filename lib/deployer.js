...
module.exports = function(args) {
  ...
  input_headers = JSON.parse(args.headers).list
  if (_.isArray(input_headers)) {
      async.eachSeries(input_headers, function(item, next) {
          ...
          readdirp({root: publicDir, entryType: 'both', fileFilter: item.fileFilter})
              .pipe(s3sync(db, syncinput).on('data', function (file) {
                  log.info(file.path + ' -> ' + file.url + ' -> ' + JSON.stringify(syncinput.headers))
              }).on('end', function () {
                  next();
